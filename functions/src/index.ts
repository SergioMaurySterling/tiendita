...
// FCM chats
export const fcmChats = exports.newMessageNotification = functions.firestore
...
//FCM historial
export const fcmHistorial = exports.newMessageNotification = functions.firestore
    .document('Pay/{userUid}')
    .onUpdate(async event => {
        const data = event.after.data();

        const uid = data.userUid
        const emisorUid = data.empUid
        ...
        const payload = {
            notification: {
                title: 'Nuevo mensaje en el chat',
                body: `${name} actualizo tu pedido a: ${status}`,
                icon: 'https://petti-2d60d.web.app/assets/logo-amarillo.svg',
                click_action: 'FCM_PLUGIN_ACTIVITY'
            },
            data: {
